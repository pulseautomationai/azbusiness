// BACKUP: Complex schema causing TypeScript recursion issues
// This is the original complex schema that was causing compilation errors
// We'll restore this once we fix the type issues
...
export default defineSchema({
  ...
  users: defineTable({
    ...
    // Admin role system for Phase 5
    role: v.optional(v.union(v.literal("user"), v.literal("admin"), v.literal("super_admin"))),
    adminPermissions: v.optional(v.array(v.string())), // Specific admin permissions
  ...
  businesses: defineTable({
    ...
    claimedByUserId: v.optional(v.string()), // User who claimed the business
    claimedAt: v.optional(v.number()), // When business was claimed
    claimed: v.boolean(),
    verified: v.boolean(),
    active: v.boolean(),
  ...
  // Simplified business content table
  ...
  analyticsEvents: defineTable({
    businessId: v.optional(v.id("businesses")), // Make optional for non-business events
    eventType: v.union(
      v.literal("page_view"),
      ...
      v.literal("cta_click"),
      v.literal("business_search"),
      v.literal("business_view"),
      v.literal("category_view"),
      v.literal("profile_view"),
      v.literal("analytics_view")
    ),
    sourceUrl: v.optional(v.string()), // Referrer or source page
    deviceType: v.optional(v.union(v.literal("mobile"), v.literal("desktop"), v.literal("tablet"))),
    userAgent: v.optional(v.string()),
    ipAddress: v.optional(v.string()), // For geographic insights
    sessionId: v.optional(v.string()), // To track user journey
